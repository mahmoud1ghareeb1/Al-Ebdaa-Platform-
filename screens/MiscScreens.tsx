...
const ImageViewer: React.FC<{ imageUrl: string; onClose: () => void }> = ({ imageUrl, onClose }) => {
    return (
        <div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center p-4" onClick={onClose} role="dialog" aria-modal="true">
            <button onClick={onClose} className="absolute top-4 right-4 text-white hover:text-slate-300 z-50" aria-label="Close image viewer">
        ...
        return (
            <>
                ...
                <div className="flex items-center mb-6">
                    <button onClick={handleBackToSets} className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors" aria-label="Back to photo sets">
                        <ArrowRightIcon className="w-6 h-6"/>
                    </button>
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200 mr-4">{selectedSet.title}</h2>
                </div>
                {loadingPhotos ? <Spinner /> : (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {photos.length > 0 ? photos.map(photo => (
                            ...
                            <p className="col-span-full text-center text-slate-500 dark:text-slate-400 py-10">لا توجد صور في هذه المجموعة.</p>
                        )}
                    </div>
                )}
            </>
